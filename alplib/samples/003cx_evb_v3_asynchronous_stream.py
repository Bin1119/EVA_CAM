    ...
    if device.apsModeIndex() > -1:
        ...
        while device.isOpened():
            ...
            for it in frameslists:
                ...
                # 转换为 OpenCV Mat 数据
                frame = it.frame()
                img = convertFrameTo8bit(frame, it.bits())
                cv.imshow("APS", img)
                cv.waitKey(1)
                break
...
def convertFrameTo8bit(frame, depth):
    """
    将输入的 16 位单通道图像转换为 8 位图像。
    
    :param frame: 输入的 16 位单通道图像 (numpy.ndarray)
    :param depth: 输入图像的位深度 (10 或 12)
    :return: 转换后的 8 位图像 (numpy.ndarray)
    """
    if frame.dtype == np.uint16:  # 检查是否为 16 位单通道图像
        if depth == 10:
            # 10 位图像转换到 8 位
            frame_8bit = cv.convertScaleAbs(frame, alpha=(255.0 / 1023.0))
        elif depth == 12:
            # 12 位图像转换到 8 位
            frame_8bit = cv.convertScaleAbs(frame, alpha=(255.0 / 4095.0))
        else:
            # 默认转换为 8 位图像
            frame_8bit = cv.convertScaleAbs(frame)
        
        return frame_8bit
    else:
        raise ValueError("输入图像不是 16 位单通道图像")
