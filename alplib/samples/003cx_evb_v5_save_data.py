...
##
 ...
 #
 # ##八.  数据存储##
 #    - 8.1 获取设备属性：调用 `getDeviceAttribute()` 方法获取设备属性。
 #    - 8.2 获取 APS 或 EVS 属性：调用 `getApsAttribute()` 或 `getEvsAttribute()` 方法获取
 #    APS 或 EVS 属性。
 #    - 8.3 创建数据写入实例：实例化数据写入对象，准备执行数据存储操作。
 #    - 8.4 打开存储文件：调用 `open()` 方法打开指定的文件以进行数据写入。
 #    - 8.5 写入数据：调用 `write()` 方法将数据写入文件。
 #    - 8.6 关闭文件：调用 `close()` 方法关闭文件并完成存储操作。
 #
 # ##九.  停止数据流##：
...
def chooseModel():
    ...
    if strcmd == "aps":
        return "NORMAL_V2", "", os.path.join(current_dir, "../../../../config/EVB/APX003CE_MOD/003ce_pure_aps_linear_master_bitformat_v5.0.data")
    elif strcmd == "evs":
        return "", "NORMAL_V2", os.path.join(current_dir, "../../../../config/EVB/APX003CE_MOD/003ce_hvs_linear_master_bitformat_v5.0.data")
    else:
        return "NORMAL_V2", "NORMAL_V2", os.path.join(current_dir, "../../../../config/EVB/APX003CE_MOD/003ce_hvs_linear_master_bitformat_v5.0.data")
...
def apsFrameCallback(device, writer_file):
    """
    该函数用于处理从设备接收到 APS 的数据帧

    :param device: 设备实例
    :param writer_file: 文件写入实例
    """
...
def evsFrameCallback(device, writer_file):
    """
...
def getCurrentTime():
    ...
    final_formatted_time = formatted_time + milliseconds_str
    return final_formatted_time

def initRecord(device,writer_file,mode):
    """
    初始化保存数据，需要填保存数据的属性

    :param device: 设备实例
    :param writer_file: 文件写入实例
    :param mode: 'h' 保存 hdf5文件 'b' 保存bin文件
    ...
    if mode == 'h':
        file_name = "RecordAlp/" + current_time + ".alpdata"
        writer_type = WriterType.HDF5
    else:
        file_name = "RecordBin/" + current_time + "/"
    ...
    # 如果 APS 模式索引有效，则初始化 APS 属性
    if device.apsModeIndex() > -1:
        # 8.2 获取 APS 属性
        aps_ptr = device.getApsAttribute()

    # 如果 EVS 模式索引有效，则初始化 EVS 属性
    if device.evsModeIndex() > -1:
        # 8.2 获取 EVS 属性
        evs_ptr = device.getEvsAttribute()

    # 8.4 打开存储文件
    error = writer_file.open(device_attribute, aps_ptr, evs_ptr)
...
def saveData(device,writer_file,save,mode):
    """
    此函数用于保存数据

    :param device: 设备实例
    :param writer_file: 文件写入实例
    :param save: True 保存数据 False 结束保存
    :param mode: 'h' 保存 hdf5文件 'b' 保存bin文件
    """
    global is_save
    if save:
        initRecord(device,writer_file,mode)
        is_save = True
    else:
        is_save = False
        time.sleep(0.1)
...
def main():
    ...
    cfg = os.path.join(current_dir, "../../../../config/EVB/APX003CE_MOD/003ce_hvs_linear_master_bitformat_v5.0.data")  # 存储配置文件路径
    ...
    t1 = threading.Thread(target = apsFrameCallback, args = (device, writer_file))
    t2 = threading.Thread(target = evsFrameCallback, args = (device, writer_file))
    ...
    ##七.  监控设备状态##
    while device.isOpened():
        print("Enter 'q' shutdown equipment")
        print("Enter 's'or any key save data")
        print("Please enter the command: ")
        user_input = input()
        if user_input.lower() == 'q':
            saveData(device,writer_file,False,'h')
            # 5.4 停止回调数据流
            device.stopStream()
            ##八.  数据存储##
            # 8.6 关闭文件
            ...
            print("\nEnter 'h' or any key save hdf5")
            print("Enter 'b' save bin")
            print("Please enter the command: ")
            user_input = input()
            if user_input.lower() == 'b':
                saveData(device,writer_file,True,'b')
            else:
                saveData(device,writer_file,True,'h')

            print("\nYou are saving the data")
            print("Enter 's' or any key to stop saving")
            print("Please enter the command: ")
            user_input = input()
            saveData(device,writer_file,False,'h')
            print("\nEnd of saving data. ")
